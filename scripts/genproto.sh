...
function gen_for_modules(){
    ...
    for dir in $(dirs); do
        ...
        echo "protoc -I=/usr/local/include/ -I=${API_ROOT} --go_out=${go_out} --go_opt=paths=source_relative --go-grpc_out=${go_out} --go-grpc_opt=paths=source_relative --go-grpc_opt=require_unimplemented_servers=false ${API_ROOT}/${dir}/$pb_file"
        ...
        run protoc -I=/usr/local/include/ \
            -I=./api \
